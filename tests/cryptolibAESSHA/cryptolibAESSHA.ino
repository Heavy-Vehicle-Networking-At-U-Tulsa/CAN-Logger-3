...
void loop() {
  ...
  if (mmcau_sha256_initialize_output(&sha_output[0])==0)Serial.println("CAU2 hardware present");
  for (i = 0; i < 8; i++) {
    sprintf(str, "%08X ", sha_output[i]);
    Serial.print(str);
  } Serial.println();


  padding_start(&data[0],sizeof(data),CRYPTO_BIG_ENDIAN);


  t = micros();
  mmcau_sha256_update (&data[0],sizeof(data)/64+1, &sha_output[0]); // # 64-byte blocks
  t = micros() - t;
  ...
  t = micros();
  mmcau_sha256_hash_n (&data[0],sizeof(data)/64+1, &sha_output[0]); // # 64-byte blocks
  t = micros() - t;
  ...
  t = micros();
  mmcau_sha256_hash (&data[0], &sha_output[0]); // # 64-byte blocks
  t = micros() - t;
  ...
  t = micros();
  mmcau_aes_encrypt (in, keysched, AES_128_NROUNDS, cipher_text); // # 16-byte block
  t = micros() - t;
  sprintf(str, "aes %d bytes %u us  KBs  ", sizeof(in), t); Serial.print(str);
  Serial.println(1000.*sizeof(in) / t);
  mmcau_aes_decrypt (cipher_text, keysched, AES_128_NROUNDS, iv); //  decrypt test
  errs = 0;
