...
def upload(event, context):
    ...
    verified = False
    signedURL = None
    if verify_meta_data_text(meta_data_bytes):
        ...
        table = dbClient.Table("CanLogMetaData")
        table.put_item(
            Item=meta_data_dict
        )
        client = boto3.client('s3', region_name='us-east-2')
        signedURL = client.generate_presigned_post(Bucket='can-log-files',
                                                   Key=meta_data_dict["id"]
                                                  )
        print('Generated presigned post')
        

    body = {
        "message": "You've called the upload function",
        "meta_data_bytes": meta_data_bytes.decode('ascii'),
        "verified": verified,
        "event": event,
        "upload_link": signedURL
    }
    return response(200, body)